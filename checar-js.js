...
// ── index.html: extrai e valida cada bloco <script> ────────────────────────────
const html = fs.readFileSync(path.join(raiz, 'index.html'), 'utf8');
const blocos = [...html.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g)];

if (!blocos.length) {
  falhar('nenhum bloco <script> encontrado no index.html');
} else {
  blocos.forEach((b, i) => {
    try {
      new vm.Script(b[1], { filename: `index.html <script #${i + 1}>` });
      console.log(`  index.html <script #${i + 1}>: OK (${b[1].length} chars)`);
    } catch (e) {
      falhar(`index.html <script #${i + 1}>: ${e.message}`);
    }
  });
}

// ── ids usados no JS que nao existem no HTML ──────────────────────────────────
// Pega erro de digitacao em getElementById, que so apareceria em runtime.
...
if (orfaos.length) falhar('getElementById aponta para ids que nao existem: ' + orfaos.join(', '));
else console.log('  ids do getElementById: OK');
